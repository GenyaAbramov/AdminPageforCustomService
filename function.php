<?php
    ...
    function setChecked($comic_id, $value) {
        global $db;
        ...
        $sql = "UPDATE comics SET checked = " . $value . " WHERE id = " . $comic_id;
    ...
    function adminGetInfos() {
        ...
        $sql = "SELECT * FROM infos WHERE status = 1 ORDER BY id DESC";
        $result = $db->result($sql);
    ...
    function adminGetSubscribes() {
        ...
        $sql = "SELECT C.comic_name, S.ads_id, S.id, S.date, S.time FROM subscribers S, comics C WHERE C.id = S.comic_id ORDER BY S.id DESC";
        $result = $db->result($sql);
    ...
    function downloadCSV() {
        ...
        $sql = "UPDATE infos SET status = 0";
        $db->execute($sql);
        
        $objPHPExcel = new PHPExcel();
        $objPHPExcel->setActiveSheetIndex(0);
        
        $title = array("A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K");
        $objPHPExcel->getActiveSheet()->setTitle('Sheet1');
        
        $objPHPExcel->getActiveSheet()->SetCellValue('A' . $rowCount, "No");
        $objPHPExcel->getActiveSheet()->SetCellValue('B' . $rowCount, "LATITUDE");
        $objPHPExcel->getActiveSheet()->SetCellValue('C' . $rowCount, "LONGITUDE");
        $objPHPExcel->getActiveSheet()->SetCellValue('D' . $rowCount, "PUBLIC IP");
        $objPHPExcel->getActiveSheet()->SetCellValue('E' . $rowCount, "DEVICE MODEL");
        $objPHPExcel->getActiveSheet()->SetCellValue('F' . $rowCount, "ADS ID");
        $objPHPExcel->getActiveSheet()->SetCellValue('G' . $rowCount, "DEVICE TYPE");
        $objPHPExcel->getActiveSheet()->SetCellValue('H' . $rowCount, "CARRIER");
        $objPHPExcel->getActiveSheet()->SetCellValue('I' . $rowCount, "NETWORK SPEED");
        $objPHPExcel->getActiveSheet()->SetCellValue('J' . $rowCount, "DATE");
        $objPHPExcel->getActiveSheet()->SetCellValue('K' . $rowCount, "TIME");
        
        $rowCount ++;
        
        foreach($result as $row):
            $objPHPExcel->getActiveSheet()->SetCellValue('A' . $rowCount, ($rowCount - 1));
            $objPHPExcel->getActiveSheet()->SetCellValue('B' . $rowCount, $row['latitude']);
            $objPHPExcel->getActiveSheet()->SetCellValue('C' . $rowCount, $row['longitude']);
            $objPHPExcel->getActiveSheet()->SetCellValue('D' . $rowCount, $row['public_ip']);
            $objPHPExcel->getActiveSheet()->SetCellValue('E' . $rowCount, $row['device_model']);
            $objPHPExcel->getActiveSheet()->SetCellValue('F' . $rowCount, $row['ads_id']);
            $objPHPExcel->getActiveSheet()->SetCellValue('G' . $rowCount, $row['device_type']);
            $objPHPExcel->getActiveSheet()->SetCellValue('H' . $rowCount, $row['carrier']);
            $objPHPExcel->getActiveSheet()->SetCellValue('I' . $rowCount, $row['network_speed']);
            $objPHPExcel->getActiveSheet()->SetCellValue('J' . $rowCount, $row['date']);
            $objPHPExcel->getActiveSheet()->SetCellValue('K' . $rowCount, $row['time']);
        ...
        header('Content-Disposition: attachment;filename="Lupolupoinfo.csv"');
